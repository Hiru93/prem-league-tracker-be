...
- A refresh token (opaque random value, not a JWT) is generated, its **sha256 hash** stored as a new `RefreshToken` row (`familyId` = a fresh UUID for this login), and the **raw** value set as an httpOnly, `Secure`, `SameSite=Strict` cookie (`refresh_token`) with a ~30-day `Max-Age`. The raw value is never persisted anywhere — only its hash.
